...
spp_vs <- list(spp1 = c(str1 = 0.1, str2 = 0.7, str3 = 1.0),
               spp2 = c(str1 = 0.3, str2 = 0.1, str3 = 0.4),
               spp3 = c(str1 = 0.2, str2 = 0.3, str3 = 0.1),
               spp4 = c(str1 = 0.1, str2 = 0.3, str3 = 0.1))
...
max_imp <- max(spp_impact_df$impact, na.rm = TRUE)
...
for(s in spp_vec) {
  ...
  ggplot(i_df, aes(x, y)) +
    ...
    scale_fill_viridis_c(na.value = 'grey90', limits = c(0, max_imp))+
    coord_equal() +
    theme_void()
...
ggplot(chi_fe_df, aes(x, y)) +
  geom_tile(aes(fill = fv), color = 'grey30', linewidth = .75, show.legend = FALSE) +
  scale_fill_gradient(low = '#ecfeff', high = 'slateblue4', na.value = 'grey90')+
  coord_equal() +
  theme_void()

ggsave(here('rasts_raw', 'rast_fe_vulnerability.png'), height = 3, width = 3, dpi = 300)
...
ggplot(fe_spp_diff, aes(x, y)) +
  ...
  scale_fill_gradient2(high = pal[1], low = pal[3], mid = pal[2], midpoint = 0, limits = c(-.5, .5))+
  coord_equal() +
  theme_void()
