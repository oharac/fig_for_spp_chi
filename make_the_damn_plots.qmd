...
for(l in lyrs[1:4]) {
  ...
  fill_cols <- c('grey96', ifelse(l == 'spp1', '#440044', 'slateblue'))
...
for(l in lyrs[5:7]) {
  ...
  ggplot(l_df, aes(x, y)) +
    geom_tile(aes(fill = val), color = 'grey30', linewidth = .75, show.legend = FALSE) +
    scale_fill_gradientn(colors = c('grey96', 'orange', 'darkred'), limits = c(0, 1)) +
    coord_equal() +
    theme_void()
...
for(f in unique(chi_per_fe_df$fe)) {
  ...
  ggsave(here('rasts_raw', sprintf('rast_per_fe_%s_chi.png', f)), height = 3, width = 3, dpi = 300)
...
ggplot(fe_spp_diff, aes(x, y)) +
  ...
  scale_fill_gradient2(high = pal[1], low = pal[3], mid = pal[2], midpoint = 0, limits = c(-.3, .3))+
  coord_equal() +
  theme_void()
